...
class SARSAAgent(BaseAgent):
    ...
    SARSA 是一种 On-Policy 的时序差分学习算法
    更新公式: Q(s,a) = Q(s,a) + α * [r + γ * Q(s',a') - Q(s,a)]
    
    关键区别：a' 是实际采样的下一个动作，而非 max_a' Q(s',a')
    这使得 SARSA 更加保守，适合处理有危险状态的环境
    """
    ...
    def update(self, state: int, action: int, reward: float,
        ...
        其中 a' 是实际采样并执行的动作（非 max_a' Q(s',a')）
        ...
        Args:
            ...
            reward: 获得的奖励
            next_state: 下一个状态
            next_action: 实际选择的下一个动作（On-Policy 关键）
            done: 是否终止
        """
...
if __name__ == '__main__':
    ...
    # 模拟 SARSA 更新（需要 next_action）
    state = 0
    action = agent.select_action(state, training=True)
    next_state = 4
    next_action = agent.select_action(next_state, training=True)
    agent.update(0, action, 0, next_state, next_action, False)
    
    state = 4
    action = next_action
    next_state = 8
    next_action = agent.select_action(next_state, training=True)
    agent.update(state, action, 0, next_state, next_action, False)
    
    state = 8
    action = next_action
    next_state = 9
    next_action = agent.select_action(next_state, training=True)
    agent.update(state, action, 1, next_state, next_action, True)  # 到达目标
    
    print("测试 SARSA Agent")
    agent.print_q_table()
    agent.print_policy()
